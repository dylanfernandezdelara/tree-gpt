...
There is no separate instant/thinking slug. Instant-like replies use `reasoning: { effort: "minimal" }` (shortest pass Muse supports; `none` returns HTTP 400). Keep `max_tokens` at 4096. The non-streaming path adds `exclude: true`; the streaming path omits it so the UI can show thinking live. Never store or echo `reasoning_details`: plain chat sends no `tools`, so there is no tool-use continuity to preserve, and replaying old thinking only slows later replies. The UI keeps a small display-only `reasoning` string per assistant message (persisted, truncated to 4k, never sent upstream). Accept-and-drop legacy blobs from old clients instead of rejecting them.
